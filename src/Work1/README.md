...
本项目是计算机图形学课程实验一，基于 **Taichi** 图形框架和 **NumPy** 实现了一个基础的 3D 变换渲染管线。项目通过手动构建 MVP 矩阵，将三维空间中的几何图元（三角形/正方形）投影到二维屏幕上进行显示，并支持通过键盘交互实时旋转图形。
...
| 文件 | 职责 |
|------|------|
| `config.py` | 定义窗口分辨率、相机参数（位置、FOV、近远平面）、三角形顶点坐标和颜色常量 |
